...
Decorative layers ignore pointer input and stay out of the accessibility tree. The opener now settles automatically after 4.5 seconds; animation also pauses outside the viewport and when the tab is hidden. The initial pause/resume button has been removed. Reduced-motion displays static artwork. Mobile uses fewer dust particles and dimmer ribbons. Existing personal links, portrait, work assets, navigation, contact behavior, and search URLs remain in place. The sample UI’s ratings/review numbers were demonstration content, not verified business evidence, and were replaced by a factual founder badge.
...
### Per-service hover light

All six service cards retain their existing surface at rest. Hover or keyboard focus adds a soft local glow: web blue `96 140 255`, brand violet `150 110 255`, photography green `52 190 130`, social rose `245 120 140`, AI cyan `60 200 235`, and marketing amber `215 165 80`. CSS draws both the inner tint and the outer halo; no additional assets are fetched. Overflow remains visible at card level so the halo can fade into the section. Cursor tracking is limited to fine pointers without reduced-motion preference, with at most one pending animation frame and no persistent animation loop. Keyboard focus uses a stationary glow.
